...
(() => {
    ...
    // ✅ WORKING FIX: If sidebar is open on load, toggle twice to refresh layout
    if (html.getAttribute("data-toggled") === "open" && window.innerWidth >= 992) {
        setTimeout(() => {
            toggleSidemenu(); // temporarily close
            toggleSidemenu(); // reopen → forces full recalculation
        }, 50);
    }
    ...
    function rtlFn() {
        ...
        document.querySelector("#style")?.setAttribute("href", "/assets/admin/css/bootstrap.rtl.min.css");
...
function ResizeMenu() {
    ...
    if (WindowPreSize.length > 1) {
        if (WindowPreSize[1] < 992 && WindowPreSize[0] >= 992) {
            mainContent.addEventListener("click", menuClose);
            setNavActive();
            toggleSidemenu();
            mainContent.removeEventListener("click", clearNavDropdown);
...
function toggleSidemenu() {
            ...
            switch (verticalStyle) {
                ...
                case "overlay":
                    ...
                    if (html.getAttribute("data-toggled") === "icon-overlay-close") {
                        ...
                        if (!localStorage.getItem("menolayout")) html.setAttribute("data-toggled", "icon-overlay-close");
                ...
                case "doublemenu":
                    ...
                    if (html.getAttribute("data-toggled") === "double-menu-open") {
                        html.setAttribute("data-toggled", "double-menu-close");
                        document.querySelectorAll(".slide-menu").forEach((e) => e.classList.remove("double-menu-active"));
                    } else {
                        let sidemenu = document.querySelector(".side-menu__item.active");
                        if (sidemenu) {
                            html.setAttribute("data-toggled", "double-menu-open");
                            if (sidemenu.nextElementSibling) sidemenu.nextElementSibling.classList.add("double-menu-active");
                            else html.setAttribute("data-toggled", "double-menu-close");
                        }
                    }
                    doublemenu();
                    break;
        ...
        if (html.getAttribute("data-toggled") === "close") {
            ...
            setTimeout(() => {
                ...
                window.addEventListener("resize", () => {
                    if (window.innerWidth >= 992) i.classList.remove("active");
                });
            }, 100);
        } else {
            menuClose();
    ...
    // ✅ Always save state
    localStorage.setItem("sidebarState", html.getAttribute("data-toggled") || "");
}
